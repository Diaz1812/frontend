...
export default function PositionEditPage() {
  ...
  useEffect(() => {
    const fetchPosition = async () => {
      ...
      try {
        ...
        const positionData = response.data.data || response.data; // Adjust based on API response structure
        setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await api.post(`/admin/positions/${positionId}`, submitData);
    ...
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center space-x-3">
          ...
          <h1 className="text-2xl font-bold text-white">Edit Position</h1>
        </div>
