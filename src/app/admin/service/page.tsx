...
export default function ServicePage() {
  ...
  const handleEdit: EditHandler = (item) => {
    router.push(`/admin/service/${item.id}/edit`);
  };

  interface DeleteHandler {
    (item: Service): Promise<void>;
  }

  const [confirmDeleteItem, setConfirmDeleteItem] = useState<Service | null>(null);

  const handleDeleteConfirm = async () => {
    if (!confirmDeleteItem) return;
    setLoading(true);
    try {
      await api.delete(`/admin/services/${confirmDeleteItem.id}`);
      setData((prev) =>
        prev.filter((service) => service.id !== confirmDeleteItem.id)
      );
