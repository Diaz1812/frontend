...
export default function TechnologyPage() {
  ...
  const [confirmDeleteItem, setConfirmDeleteItem] = useState<Technology | null>(null);
  ...
  const handleAdd = () => {
    router.push('/admin/technology/new');
  };

  const handleEdit = (item: Technology) => {
    router.push(`/admin/technology/${item.id}/edit`);
  };
  ...
  const handleDeleteConfirm = async () => {
    ...
    setLoading(true);
    try {
      await api.delete(`/admin/technologies/${confirmDeleteItem.id}`);
      setData(prev => prev.filter(tech => tech.id !== confirmDeleteItem.id));
        ...
        {!loading && filteredData.length === 0 ? (
          <div className="bg-gray-800 rounded-xl p-8 text-center">
            ...
            <h3 className="text-lg font-semibold text-gray-300 mb-2">
              {searchTerm ? 'No technology found' : 'No technologies yet'}
            </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredData.map((tech) => (
              <div key={tech.id} className="bg-gray-800 rounded-xl p-6 hover:bg-gray-750 transition-colors">
                {/* Image */}
                ...
                <div className="flex space-x-2">
                  ...
                  <button
                    onClick={() => setConfirmDeleteItem(tech)}
                    disabled={loading}
        ...
        {!loading && (
          <div className="mt-8 bg-gray-800 rounded-xl p-4">
            <div className="flex items-center justify-between text-sm text-gray-400">
              <span>Total Technologies: {data.length}</span>
              {searchTerm && (
